...
from Evie import BOT_ID, DEV_USERS, MONGO_DB_URI, OWNER_ID, SUDO_USERS, tbot, ubot
...
from Evie.modules.sql.chats_sql import add_chat, is_chat, rmchat
    ...
    if event.user_added:
        if event.user_id == BOT_ID:
            ...
            await tbot.send_message(
                -1001486931338, f"Evie Added to {event.chat.title}\n`{event.chat_id}`"
            )
            await tbot.send_message(
                event.chat_id,
                "Heya :-D Now leave your group on my hands and let me manage it. If you need any help, head to @EvieSupport.",
            )
...
async def stat(event):
    ...
    used = get_all_users()
    await event.reply(
        f"<b>I have <u>{len(used)}</u> Users In My Database.</b>", parse_mode="HTML"
    )
...
async def approve(event):
    ...
    if sql.is_sudo(iid):
        ...
        return
    await event.reply(
        "Sucessfully set the Disaster level of this user to **Sudo User**."
    )
...
async def msg(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.reply(
            "This is a Assembler restricted command. You do not have permissions to run this."
        )
        return
    elif event.sender_id in DEV_USERS:
        await event.reply(
